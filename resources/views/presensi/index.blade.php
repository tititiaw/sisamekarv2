<x-app-layout>
    <x-slot name="header">
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900 dark:text-gray-100">
                        <!-- Header dengan Tanggal -->
                        <div class="flex justify-between items-center mb-6">
                            <h1 class="text-2xl font-bold">
                                Data Presensi
                            </h1>
                            <div class="flex items-center space-x-2">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                    <path fill-rule="evenodd" d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z" clip-rule="evenodd" />
                                </svg>
                                <span>{{ now()->isoFormat('dddd, D MMMM YYYY') }}</span>
                            </div>
                        </div>
    
                        <!-- Statistik Cards -->
                        <div class="grid gap-4 mb-6 md:grid-cols-2 lg:grid-cols-4">
    
                            <div class="bg-white dark:bg-gray-700 overflow-hidden shadow-sm rounded-lg">
                                <div class="p-4">
                                    <div class="flex items-center">
                                        <div class="p-3 rounded-full bg-green-100 dark:bg-green-900">
                                            <svg class="h-6 w-6 text-green-600 dark:text-green-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                                            </svg>
                                        </div>
                                        <div class="ml-4">
                                            <div class="text-sm font-medium text-gray-500 dark:text-gray-400">Hadir</div>
                                            <div class="text-lg font-semibold">{{ $totalHadir }}</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
    
                            <div class="bg-white dark:bg-gray-700 overflow-hidden shadow-sm rounded-lg">
                                <div class="p-4">
                                    <div class="flex items-center">
                                        <div class="p-3 rounded-full bg-yellow-100 dark:bg-yellow-900">
                                            <svg class="h-6 w-6 text-yellow-600 dark:text-yellow-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                            </svg>
                                        </div>
                                        <div class="ml-4">
                                            <div class="text-sm font-medium text-gray-500 dark:text-gray-400">Sakit/Izin</div>
                                            <div class="text-lg font-semibold">{{ $totalSakitIzin }}</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
    
                            <div class="bg-white dark:bg-gray-700 overflow-hidden shadow-sm rounded-lg">
                                <div class="p-4">
                                    <div class="flex items-center">
                                        <div class="p-3 rounded-full bg-red-100 dark:bg-red-900">
                                            <svg class="h-6 w-6 text-red-600 dark:text-red-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                                            </svg>
                                        </div>
                                        <div class="ml-4">
                                            <div class="text-sm font-medium text-gray-500 dark:text-gray-400">Alpa</div>
                                            <div class="text-lg font-semibold">{{ $totalAlpa }}</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
    
                        <!-- Tabel Presensi -->
                        <!-- ... kode tabel yang sudah ada ... -->
                    </div>
                </div>
            </div>
        </div>
    </x-slot>

    

    <x-content>
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900 dark:text-gray-100">
                        <div class="mb-4 flex gap-2">
                            <a href="{{ route('presensi.create') }}">
                                <x-secondary-button>Tambah Data</x-secondary-button>
                            </a>  
                            <a href="{{ route('presensi.export') }}">
                                <x-secondary-button>Export Data</x-secondary-button>
                            </a>
                        </div>

                        @if(session('success'))
                            <div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative mb-4">
                                {{ session('success') }}
                            </div>
                        @endif

                        @if(session('error'))
                            <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mb-4">
                                {{ session('error') }}
                            </div>
                        @endif

                        <x-table>
                            <x-slot:thead>
                                <tr>
                                    <th class="p-3">No</th>
                                    <th class="p-3">Nama Siswa</th>
                                    <th class="p-3">NIS</th>
                                    <th class="p-3">Jenis Kelamin</th>
                                    <th class="p-3">Kelas</th>
                                    <th class="p-3">Tanggal Absensi</th>
                                    <th class="p-3">Status Kehadiran</th>
                                    <th class="p-3">Aksi</th>
                                </tr>
                            </x-slot:thead>
                            <tbody>
                                @forelse($presensis as $presensi)
                                    <tr class="border-b">
                                        <td class="p-3">{{ $loop->iteration }}</td>
                                        <td class="p-3">{{ $presensi->name }}</td>
                                        <td class="p-3">{{ $presensi->nis }}</td>
                                        <td class="p-3">
                                            {{ $presensi->gender == 'B' ? 'Laki-laki' : 'Perempuan' }}
                                        </td>
                                        <td class="p-3">{{ $presensi->kelas->name }}</td>
                                        <td class="p-3">{{ \Carbon\Carbon::parse($presensi->tanggal)->format('d/m/Y') }}</td>
                                        <td class="p-3">
                                            @switch($presensi->status_kehadiran)
                                                @case('Hadir')
                                                    <span class="px-2 py-1 bg-green-100 text-green-800 rounded">Hadir</span>
                                                    @break
                                                @case('Sakit')
                                                    <span class="px-2 py-1 bg-yellow-100 text-yellow-800 rounded">Sakit</span>
                                                    @break
                                                @case('Izin')
                                                    <span class="px-2 py-1 bg-blue-100 text-blue-800 rounded">Izin</span>
                                                    @break
                                                @case('Alpa')
                                                    <span class="px-2 py-1 bg-red-100 text-red-800 rounded">Alpa</span>
                                                    @break
                                            @endswitch
                                        </td>
                                        <td class="p-3">
                                            <div class="flex gap-2">
                                                <a href="{{ route('presensi.edit', $presensi->id) }}" 
                                                   class="text-blue-500 hover:text-blue-700">
                                                    <i class="fas fa-edit" title="Edit"></i>
                                                </a>
                                                <form action="{{ route('presensi.destroy', $presensi->id) }}" 
                                                      method="POST" 
                                                      onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini?');">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="text-red-500 hover:text-red-700">
                                                        <i class="fas fa-trash" title="Hapus"></i>
                                                    </button>
                                                </form>
                                            </div>
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="8" class="p-3 text-center">
                                            Tidak ada data presensi
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </x-table>

                        <div class="mt-4">
                            {{ $presensis->links() }}
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </x-content>
</x-app-layout>